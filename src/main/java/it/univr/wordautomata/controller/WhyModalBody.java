...
/**
 * Represents the body of the dialog that shows the reasons why the automaton is
 * rejecting the word.
...
public class WhyModalBody extends VBox {
        ...
        private void initCircles() {
                ...
                PathFinder.consumedAllWordProperty().addListener(
                                (observable, oldValue, newValue) -> setCircleClass(consumedEntireWordIndicator,
                                                newValue));
                PathFinder.endedOnFinalStateProperty().addListener(
                                (observable, oldValue, newValue) -> setCircleClass(endedOnFinalStateIndicator,
                                                newValue));
                Model.getInstance().atLeastOneEdgeProperty().addListener(
                                (observable, oldValue, newValue) -> setCircleClass(atLeastATransitionIndicator,
                                                newValue));
                Model.getInstance().initialStateProperty().addListener(
                                (observable, oldValue, newValue) -> setCircleClass(singleInitialStateIndicator,
                                                newValue != null));
